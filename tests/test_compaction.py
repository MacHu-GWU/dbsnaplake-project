...
def test_calculate_merge_plan():
    ...
    for _ in range(1, 90):
        i += 1
        files.append(File(id=f"f-{i}", size=random.randint(1, 5)))
    for _ in range(10):
        i += 1
        files.append(File(f"f-{i}", size=random.randint(50, 100)))
    file_groups = calculate_merge_plan(files, target_size=64)
    # for file_group in file_groups:
    #     print(file_group.total_size, [file.size for file in file_group.files])

    files = list()
    i = 0
    for _ in range(1, 100):
        i += 1
        files.append(File(id=f"f-{i}", size=random.randint(32, 128)))
    file_groups = calculate_merge_plan(files, target_size=64)
    # for file_group in file_groups:
    #     print(file_group.total_size, [file.size for file in file_group.files])
